...
function AdminLine() {
  ...
  const onSelectSubwayLine = (event) => {
    const $target = event.target;
    const $subwayLineItem = $target.closest(".subway-line-item");
    const isSelectSubwayLineItem = $target.classList.contains(
      "subway-line-item"
    );
    if (isSelectSubwayLineItem) {
      api.line.getById($subwayLineItem.dataset.lineId).then((data) => {
        document.querySelector(
          ".lines-info"
        ).innerHTML = lineInformationTemplate(data);
      });
    }
  };
  ...
  const initEventListeners = () => {
    ...
    $createSubwayLineButton.addEventListener(EVENT_TYPE.CLICK, onSubmitHandler);
  };
